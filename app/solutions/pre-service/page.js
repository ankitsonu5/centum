...
export default function PreServicePage() {
  const steps = [
    { 
      q: "Accuracy of Data", 
      a: "We implement strict Data Integrity by collecting the patient's full legal name and cross check this with their insurance ID, current address, and date of birth. We also scan the drivers licence & insurance card details into the EHR system, as against manual typing.", 
      step: "01" 
    },
    { 
      q: "Eligibility Verification", 
      a: "We use automated tools to query the payer’s database immediately upon scheduling and again 24 hours before the appointment. We check for important details such as coverage validity, plan exclusions, and deductible status.", 
      step: "02" 
    },
    { 
      q: "Obtain Prior Authorisations", 
      a: "Specific procedures require permission in advance. Failure can often lead to claim denial with no recourse for appeal. We obtain the Prior Authorisation number by invoking specific CPT codes against the patients insurance policy.", 
      step: "03" 
    },
    { 
      q: "Financial Transparency", 
      a: "Chasing patients for money post procedures is costly. We calculate the estimated patient responsibility (copay + remaining deductible) so that the patient can take an informed decision.", 
      step: "04" 
    }
  ];
        ...
        >
          ...
          <div className="relative z-10 text-center text-white px-6 w-full max-w-5xl mx-auto">
            <h1 className="text-4xl md:text-6xl font-semibold tracking-tight md:tracking-wide drop-shadow-md">
              Pre-Service: Patient Registration & Insurance Verification
            </h1>
          </div>
        </div>
